...
public SqlConnect() {
	...
	public ResultSet find(String select) throws SQLException {
		...
		if (select.equals("*"))
			sql = "select name �W��,status ���A,user �ϥΪ�,notice �Ƶ�,beanaccount �֨��b��,beanpassword �֨��K�X from account";
		else
			sql = "select name �W��,status ���A,user �ϥΪ�,notice �Ƶ�,beanaccount �֨��b��,beanpassword �֨��K�X from account where status='" + select + "';";
	...
	public int update(String name, String status, String user, String notice) throws SQLException {
		...
		String updateStr;
		if (status.equals("������") || status.equals("�m����"))
			updateStr = "UPDATE account SET status='" +  status + "', user='" + user + "', notice = '" + notice + "' WHERE name='" + name + "';";
		else
			updateStr = "UPDATE account SET status='" +  status + "', user='�L', notice = '" + notice + "' WHERE name='" + name + "';";
